 ...
 * @CreatedDate           : 2025-11-24 11:31:25                              *
 * @LastEditors           : adolnb<loro.lorenzonunez@gmail.com>              *
 * @LastEditDate          : 2025-11-28 09:46:37                              *
 * @FilePath              : page.tsx                                         *
...
export default function VehiclesPage() {
    ...
    const { vehicles, setVehicles, loading, } = useVehicle();
        ...
        <main className='min-h-screen flex flex-col'>
            ...
            <div className='py-8 px-15 my-5'>
                <div className='flex justify-between items-center mb-6'>
                    <h1 className="text-title text-[var(--primary)] text-xl md:text-2xl lg:text-3xl">Vehiculos</h1>
        ...
        wheelchairAccessible: boolean;
                        ...
                        <tbody className="divide-y divide-[var(--divider)] border border-[var(--border-hover)]">
                            ...
                            {!loading && vehicles.length === 0 && (
                                <tr>
                                    <td colSpan={6} className="text-center py-4">No hay vehiculos disponibles.</td>
                                </tr>
                            )}

                            {!loading && vehicles.length > 0 && (vehicles.map(veh => (
                                <tr key={veh._id}>
                                    <td className="px-4 py-3">{veh.plate}</td>
                                    <td className="px-4 py-3">{veh.vehicleType}</td>
                                    <td className="px-4 py-3">{veh.color}</td>
                                    <td className="px-4 py-3 capitalize">{veh.status}</td>
                                    <td className="px-4 py-3 capitalize">{veh.driverId || "Fuera de servicio"}</td>
                                    <td className="flex px-4 py-3 gap-2">
                                        {/* <ButtonComponent variant='secondary' color='#EDC911' onClick={() => openModal({
                                            title: `Editar Vehiculo - ${emp.name}`,
                                            size: "lg",
                                            children: (
                                                <UpdateEmployee ref={formRef} employee={emp} onSuccess={(updated: any) => {
                                                    setVehicles(prev => prev.map(e => e._id === updated._id ? updated : e));
                                                    closeModal();
                                                }}/>
                                            ),
                                            onConfirm: () => {
